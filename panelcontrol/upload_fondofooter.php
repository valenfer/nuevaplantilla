...
try {
    ...
    $target_file = $target_dir . "fondo-footer.png";
    ...
    if ($_FILES["fondoFooter"]["error"] !== UPLOAD_ERR_OK) {
        echo json_encode([
            "success" => false, 
            "message" => "Error en la subida del archivo: " . $_FILES["fondoFooter"]["error"],
            "debug" => $log
        ]);
        exit;
    }
    ...
    $check = getimagesize($_FILES["fondoFooter"]["tmp_name"]);
    if ($check === false) {
        echo json_encode([
            "success" => false, 
    ...
    if (move_uploaded_file($_FILES["fondoFooter"]["tmp_name"], $target_file)) {
        echo json_encode([
            "success" => true, 
